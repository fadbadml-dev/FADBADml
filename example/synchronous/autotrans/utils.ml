...
let ifind v vec =
  Array.fold_left (fun i vec_i -> if MyOp.(vec_i <? v) then i + 1 else i) 0 vec

(* linear interpolatiion, x1 <= xi <= x2 *)
let interp (x1 : MyOp.t) (x2 : MyOp.t) (val1 : MyOp.t) (val2 : MyOp.t) xi : MyOp.t =
  let open MyOp in
...
let extrap (x1 : MyOp.t) (x2 : MyOp.t) (val1 : MyOp.t) (val2 : MyOp.t) xi : MyOp.t =
  let open MyOp in
...
let lookup1D (line, vals) =
  let line = Array.map MyOp.make line in
  let vals = Array.map MyOp.make vals in
  fun l ->
  try
...
let lookup2D (line, col, vals) =
  let line = Array.map MyOp.make line in
  let col = Array.map MyOp.make col in
  let vals = Array.map (Array.map MyOp.make) vals in
  fun (l, c) ->
  try
    let li = ifind l line in
